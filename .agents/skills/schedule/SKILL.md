...
The backlog contains an activation item named `Daily LinkedIn scheduler
sentinel`. The enable/disable scripts open or close it because Noodle 0.1.5
wakes the scheduler only while a backlog item is open. Never schedule, execute,
edit, or complete that sentinel yourself. It is not work; the daily standalone
order below is the only work this scheduler may create.
