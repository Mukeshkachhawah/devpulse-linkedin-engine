...
**Embedded / lakehouse-style vector database.**
Stores vectors alongside multi-modal data; strong for local-to-production paths and analytics-friendly workflows.
Often used with Arrow/Parquet-style data lakes.

**Best for:**
Python-centric stacks, multimodal embeddings, teams that want embedded + scalable file-based storage without operating a heavy cluster on day one.

### Supabase (pgvector)

**Managed Postgres platform with pgvector enabled.**
Same mental model as pgvector, but with hosted auth, APIs, and dashboard familiar to fullstack builders.

**Best for:**
Startups already on Supabase; RAG prototypes that should share SQL + vectors in one place; row-level security patterns for multi-tenant apps.

### MongoDB Atlas Vector Search

**Vector search inside MongoDB Atlas.**
Keeps documents and embeddings in one document database; filters use familiar Mongo query style.

**Best for:**
Teams standardized on MongoDB; content already in document collections; Atlas-managed ops instead of a separate vector service.
...
- **Also know** = LanceDB (embedded/lakehouse), Supabase pgvector (managed Postgres), MongoDB Atlas Vector Search (document + vectors)
