...
**Repetition controls (penalties):**
Models can get stuck repeating phrases ("as well as as well as...").
**Frequency penalty:** Penalize tokens proportional to how often they already appeared in the generation — reduces loops.
**Presence penalty:** Penalize a token if it appeared at all — pushes toward new topics/vocabulary.
**Repetition penalty (common in open-source stacks):** Down-weight tokens that already occurred; implementation details vary by library.
Use carefully: too high → awkward wording and topic drift; too low → sticky repetition. Creative writing may use mild penalties; code/JSON usually prefers low temperature + schema instead.
...
- **Top-k / Top-p** = truncate sampling distribution; prefer with temperature
- **Repetition / frequency / presence penalties** = reduce loops and overused tokens; tune carefully
