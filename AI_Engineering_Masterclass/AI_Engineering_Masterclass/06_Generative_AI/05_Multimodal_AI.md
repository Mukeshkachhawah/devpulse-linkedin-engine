...
## Implementing Multimodal Apps — APIs and Frameworks

**Common API building blocks:**
- **Vision:** OpenAI Vision / GPT-4o image inputs; Claude image inputs; Gemini multimodal
- **Image generation:** DALL-E API, other diffusion APIs
- **Speech-to-text:** Whisper API (and hosted ASR alternatives)
- **Text-to-speech:** provider TTS APIs (OpenAI TTS, ElevenLabs, etc.)
- **Open models:** Hugging Face multimodal checkpoints when you self-host

**Orchestration:**
- **LangChain for multimodal apps:** loaders + chains that mix image/audio inputs with tools
- **LlamaIndex for multimodal apps:** index images/docs together; retrieve across modalities

**Design tips:**
- Budget visual/audio tokens against the text context window
- Log modality provenance in traces (observability)
- Safety: treat uploaded images/files as untrusted context (Module 14)
